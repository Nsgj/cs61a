...
def interleave(s0, s1):
    ...
    if s0 == empty:
        ...
        return s0
    return link(s0[0],interleave(s1,rest(s0)))
...
def height(t):
    ...
    for i in subtrees(t):
        height(i)
    return max([1 + height(x) for x in subtrees(t)])
...
def sprout_leaves(t, vals):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        return tree(entry(t),[tree(x) for x in vals])
    else:
        sub = [sprout_leaves(x,vals) for x in subtrees(t)]
        return tree(entry(t),sub)
